...
async function normalizeImageBuffer(buffer) {
  try {
    // اختبار حقيقي لفك الصورة، مو metadata فقط
    await sharp(buffer)
      .rotate()
      .jpeg({ quality: 100 })
      .toBuffer()
    ...
    try {
      ...
      console.log('Sharp decode error:', sharpErr.message)
      console.log('HEIC convert error:', heicErr.message)

      throw new Error('صيغة الصورة غير مدعومة على السيرفر، جرّب تحويل الصورة إلى JPG أو PNG')
    }
  }
}
...
async function convertImageToWebp(buffer) {
  ...
  const metadata = await validateImage(normalizedBuffer)

  let outputBuffer = await sharp(normalizedBuffer)
    .rotate()
    .webp({
